...
public class ControladorRegistrarCompra implements ActionListener{
	...
	private TableModel tbInggredientesModel;
	...
	public void actionPerformed(ActionEvent e) {
		...
		String tiraSQL;
		this.Registrarcompra.getMensaje().showMessageDialog(this.Registrarcompra.getTxtIngrediente(), actionCommand);
		if(actionCommand.equals("Registrar")){
			if(Registrarcompra.getTxtIngrediente().getText().equals("") || Registrarcompra.getTxtCantidad().getText().equals("")||Registrarcompra.getTxtNombre().getText().equals("")){
				...
				try {	
					tiraSQL="SELECT * FROM ingrediente where codigo='"+Registrarcompra.getTxtIngrediente().getText()+"' or nombre='"+this.Registrarcompra.getTxtNombre().getText()+"' and estatus='A'";
					ResultSet resultSet1 = Conexion.consultar(tiraSQL);
					if(!resultSet1.next()){
						tiraSQL="INSERT INTO ingrediente (codigo,nombre,stock,estatus) VALUES ('"+Registrarcompra.getTxtIngrediente().getText()+"','"+Registrarcompra.getTxtNombre().getText()+"','"+Registrarcompra.getTxtCantidad().getText()+"','A')";
						Conexion.ejecutar(tiraSQL);
						Registrarcompra.getMensaje().showMessageDialog(Registrarcompra.getBtRegistrar(), "Ingrediente Registrado");
					}
					else{
						tiraSQL="UPDATE ingrediente set stock='"+(Float.parseFloat(Registrarcompra.getTxtCantidad().getText())+Float.parseFloat(resultSet1.getString("stock")))+"' where codigo='"+Registrarcompra.getTxtIngrediente().getText()+"' or nombre='"+this.Registrarcompra.getTxtNombre().getText()+"'and estatus='A'";
						this.Registrarcompra.getMensaje().showMessageDialog(this.Registrarcompra.getBtRegistrar(), "Stock Actualizado");
						Conexion.ejecutar(tiraSQL);
					}
					LimpiarPantalla();
					CargarIngrediente();
	...
	public void CargarIngrediente(){
		...
		tbInggredientesModel = 	new DefaultTableModel(datos,columnas);
